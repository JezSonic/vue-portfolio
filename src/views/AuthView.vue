...
<div class="login-form">
    <h2>Login</h2>
    <input type="email" v-model="login_email" placeholder="Email..."/>
    ...
    <Button :disabled="login_email.length == 0 || login_password.length == 0" :text="'Submit'" @click="login" />
    <h2>...or create an account</h2>
    <input type="email" v-model="register_email" placeholder="Email..."/>
    <p v-if="errors !== null && errors['email']" v-for="(item, index) in errors['email']" :key="index">{{ item }}</p>
    <input type="password" v-model="register_password" placeholder="Password..."/>
    <p v-if="errors !== null && errors['password']" v-for="(item, index) in errors['password']" :key="index">{{ item }}</p>
    <Button :disabled="register_email.length == 0 || register_password.length == 0" :text="'Submit'" @click="register" />
    <h2>Authenticate using social accounts</h2>
    <div class="social-accounts">
        <Button text="Google" @click="oauth(OAuthProvider.Google)" />
        <Button text="GitHub" @click="oauth(OAuthProvider.GitHub)"/>
    </div>
</div>
</template>
