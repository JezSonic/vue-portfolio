        ...
        <div v-else-if="userData != null" class="text-gray-900 dark:text-white w-full max-w-6xl mx-auto px-4 py-8">
                ...
                <div class="flex flex-col md:flex-row items-center md:items-start gap-6">
                    <div class="relative z-0">
                        ...
                        <div class="absolute -bottom-2 -right-2 bg-gray-900 rounded-full p-1 shadow-lg" v-else>
                            <span
                                class="text-xs px-2 py-1 bg-gray-600 text-white rounded-full">{{ t("userProfileView.profileStatus.private")
                                }}</span>
                        </div>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-b-lg shadow-lg overflow-hidden">
                <div class="p-6 border-b border-gray-300 dark:border-gray-700">
                    <h2 v-once class="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="bg-gray-750 rounded-lg p-4 shadow-inner">
                            ...
                            <div class="space-y-3">
                                <div>
                                    <p v-once class="text-sm font-medium text-gray-500 dark:text-gray-400">
                                        {{ t("userProfileView.fields.fullName") }}</p>
                                    <p class="text-gray-800 dark:text-gray-200 truncate max-w-full">{{ userData?.name
                                        }}</p>
                                </div>
                                <div>
                                    <p v-once class="text-sm font-medium text-gray-500 dark:text-gray-400">
                                        {{ t("userProfileView.fields.emailAddress") }}</p>
                                    <p class="text-gray-800 dark:text-gray-200 truncate max-w-full">{{ userData?.email
                                        }}</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="bg-gray-750 rounded-lg p-4 shadow-inner" v-if="userData?.github">
                            ...
                            <div class="mt-4 grid grid-cols-2 gap-2"
                                 v-if="userData?.github?.public_repos || userData?.github?.followers">
                                <div class="bg-gray-200 dark:bg-gray-800 rounded p-2 text-center">
                                    <p class="text-xl font-bold text-gray-800 dark:text-gray-200">
                                        {{ userData?.github?.public_repos }}</p>
                                    <p v-once class="text-xs text-gray-500 dark:text-gray-400">
                                        {{ t("userProfileView.connectedAccounts.repositories") }}</p>
                                </div>
                                <div class="bg-gray-200 dark:bg-gray-800 rounded p-2 text-center">
                                    <p class="text-xl font-bold text-gray-800 dark:text-gray-200">
                                        {{ userData?.github?.followers }}</p>
                                    <p v-once class="text-xs text-gray-500 dark:text-gray-400">
                                        {{ t("userProfileView.connectedAccounts.followers") }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
